/****************************************************************************
 * arch/risc-v/src/jx009/jx_rv_irq_dispatch.c
 *
...
static void *riscv_dispatch_irq_ext(uintreg_t irq, uintreg_t *regs)
{
  ...
  uintptr_t aplic_base = RISCV_APLIC_IDC(JX009_APLIC_BASE, hartid) +
...
static void *riscv_dispatch_irq_ext(uintreg_t irq, uintreg_t *regs)
{
  ...
  while ((extirq = getreg32(JX009_PLIC_CLAIM)) != 0)
    {
      regs = riscv_doirq(irq + extirq, regs);
      putreg32(extirq, JX009_PLIC_CLAIM);
    }
