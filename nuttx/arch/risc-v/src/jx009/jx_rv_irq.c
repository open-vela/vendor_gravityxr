/****************************************************************************
 * arch/risc-v/src/jx009/jx_rv_irq.c
 *
...
#ifdef CONFIG_RPTUN
static int jx_ipi_handler(int mcause, void *regs, void *args)
{
...
void up_irqinitialize(void)
{
  ...
  putreg32(0x0, JX009_PLIC_ENABLE1);
  putreg32(0x0, JX009_PLIC_ENABLE2);
#else
  riscv_aplic_disable_irqs(JX009_RV_APLIC_BASE, JX009_RV_APLIC_NR_IRQ);
#endif
  ...
  for (id = 0; id < JX009_RV_APLIC_NR_IRQ; id++)
    {
      riscv_aplic_configure_irq(JX009_RV_APLIC_BASE, id + 1, 0, id + 1);
    }

  riscv_aplic_init_msi(JX009_RV_APLIC_BASE, JX009_RV_IMSIC_BASE, 0, 3, 0, 0);
#elif defined(CONFIG_ARCH_RV_HAVE_APLIC)
  /* Set default IRQ target hart index to 0 and priority */

  for (id = 0; id < JX009_RV_APLIC_NR_IRQ; id++)
    {
      riscv_aplic_configure_irq(JX009_RV_APLIC_BASE, id + 1,
                                RISCV_APLIC_DEFAULT_PRIORITY, 0);
    }

  riscv_aplic_init(JX009_RV_APLIC_BASE, RISCV_APLIC_ENABLE_IDELIVERY,
  ...
  for (id = 1; id <= 52; id++)
    {
      putreg32(1, (uintptr_t)(JX009_PLIC_PRIORITY + 4 * id));
    }

  /* Set irq threshold to 0 (permits all global interrupts) */

  putreg32(0, JX009_PLIC_THRESHOLD);
#endif
  ...
  if (irq == RISCV_IRQ_SOFT)
  ...
  else if (irq > RISCV_IRQ_EXT)
    {
      ...
      if (0 <= extirq && extirq <= 63)
        {
#ifndef CONFIG_ARCH_RV_HAVE_APLIC
          modifyreg32(JX009_PLIC_ENABLE1 + (4 * (extirq / 32)),
                      1 << (extirq % 32), 0);
#else
          riscv_aplic_disable_irq(JX009_RV_APLIC_BASE, extirq);
#endif
#ifdef CONFIG_ARCH_RV_HAVE_IMSIC
      ...
      if (0 <= extirq && extirq <= 63)
        {
          ...
          modifyreg32(JX009_PLIC_ENABLE1 + (4 * (extirq / 32)),
                      0, 1 << (extirq % 32));
#else
          riscv_aplic_configure_irq(JX009_RV_APLIC_BASE, extirq,
                                    RISCV_APLIC_SOURCECFG_SM_EDGE_RISE,
                                    up_cpu_index());
#ifdef CONFIG_ARCH_RV_HAVE_IMSIC
          riscv_imsic_local_eie_enable(extirq);
#endif
          riscv_aplic_enable_irq(JX009_RV_APLIC_BASE, extirq);
#endif
        }
      else
        {
...
irqstate_t up_irq_enable(void)
{
  ...
  /* Enable APLIC irq */
#ifdef CONFIG_ARCH_RV_HAVE_IMSIC
  modifyreg32(JX009_RV_APLIC_BASE + RISCV_APLIC_DOMAINCFG, 0x0,
