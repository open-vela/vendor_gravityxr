/****************************************************************************
 * arch/risc-v/src/jx009/jx_rv_start.c
 *
...
#include "jx_rv_userspace.h"

#ifdef CONFIG_BUILD_KERNEL
#  include "jx_rv_mm_init.h"
#endif
...
/****************************************************************************
 * Name: jx_rv_clear_bss
 ****************************************************************************/

static void jx_rv_clear_bss(void)
{
  uint32_t *dest;
...
static void jx_rv_copy_data(void)
{
  const uint32_t *src;
  uint32_t *dest;
...
#ifdef CONFIG_ARCH_USE_S_MODE
static void jx_boot_secondary(int mhartid, uintptr_t dtb)
{
  int i;
...
extern int nuttx_jx_uart_init(void);

void jx_rv_start(int mhartid, const char *dtb)
{
#ifdef CONFIG_ARCH_USE_S_MODE
  /* Boot other cores */
  ...
  if (!boot_secondary)
    {
      boot_secondary = true;
      jx_boot_secondary(mhartid, (uintptr_t)dtb);
    }
#endif
  ...
  if (mhartid > 0)
    {
      //TODO: GXR JX009 A45 mhartid issue
      //goto cpux;
    }
